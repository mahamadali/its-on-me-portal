...
class Merchant extends CI_Model
{
    ...
    public function searchProduct($search_text) {
     ...
     public function ProductDetails($product_id,$brand_id) {
            ...
            $this->db->select('`products`.*, `brands`.* ,CONCAT("'.base_url().'", `products`.`id` as id ,`products`.product_image) as product_image,`merchants.physical_address` as physical_address ,`merchants.username` as merchant_name , `merchants.province` as province,provinces.name as province_name , brands.name as brand_name');
            $this->db->where('merchant_id',$brand_id);
