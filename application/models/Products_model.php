      ...
      public function getProductSizes($id)
       ...
       /*  public function searchByBrand($merchantName)
        ...
        public function searchByBrand($merchantName)
        {
            ...
            $this->db->select('`merchants`.*,CONCAT("'.base_url().'", `merchants`.profile_picture) as profile_picture,merchants.is_super_merchant as is_super_merchant, provinces.name as province_name');
            $this->db->join('provinces', 'provinces.id=merchants.province');
            if(!empty($merchantName))
            {
